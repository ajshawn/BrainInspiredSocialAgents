...
if __name__ == '__main__':
  result_path = './results/'
  figure_path = './figures/'
  rollout_pairs = ['cp7357', 'cp9651', 'AH',]
  result_dict = {}
  ...
  fig, axs = plt.subplots(len(rollout_pairs),1, figsize=(4,15), sharex=True, sharey=True)
  for i, rollout_pair in enumerate(rollout_pairs):

    plot_histograms_seaborn(axs[i],
                            result_dict['plsc1_vs_2_' + rollout_pair],
                            'PLSC1 vs 2' if i==len(rollout_pairs)-1 else None,
                            rollout_pair,
                            legends=['PLSC1', 'PLSC2', 'Both'])
  plt.tight_layout()
  plt.savefig(f'{figure_path}plsc1_vs_2.png')
  plt.show()
  ## To histogram the plsc1 vs 2 results

  fig, axs = plt.subplots(len(rollout_pairs),1, figsize=(4,15), sharex=True, sharey=True)
  for i, rollout_pair in enumerate(rollout_pairs):

    plot_histograms_seaborn(axs[i],
                            result_dict['plsc1_vs_u1_' + rollout_pair],
                            'PLSC1 vs U1' if i==len(rollout_pairs)-1 else None,
                            rollout_pair,
                            legends=['PLSC1', 'U1', 'Both'])
  plt.tight_layout()
  plt.savefig(f'{figure_path}plsc1_vs_u1.png')
  plt.show()
  ...
  fig, axs = plt.subplots(len(rollout_pairs), 4, figsize=(16, 12))
  ...
  fig, axs = plt.subplots(1,4, figsize=(15,3), sharey=True, sharex=True)
  for j, key in enumerate(['prey_self_rep_matrix', 'prey_partner_rep_matrix', 'predator_self_rep_matrix',
                           'predator_partner_rep_matrix']):
    data = []
    for i, rollout_pair in enumerate(rollout_pairs):
    ...
    axs[j].set_title(key)
    axs[j].set_xticklabels(rollout_pairs)
