...
for ii = 1:numel(video_paths)
  ...
  for prey_id = prey_ids
    ...
    for predator_id = predator_ids
      ...
      prey_score_both = computeBasicVar(xx, prey_net_data, 10, num_perm);
      prey_score_1 = computeBasicVar(xx(1), prey_net_data, 10, num_perm);
      prey_score_2 = computeBasicVar(xx(2), prey_net_data, 10, num_perm);
      ...
      predator_score_1 = computeBasicVar(xx(1), predator_net_data, 10, num_perm);
      predator_score_2 = computeBasicVar(xx(2), predator_net_data, 10, num_perm);
      predator_score_both = computeBasicVar(xx, predator_net_data, 10, num_perm);
      ...
      results.(['pair_' num2str(predator_id) '_' num2str(prey_id) '_prey']) = [prey_score_1 prey_score_2 predator_score_both];
      results.(['pair_' num2str(predator_id) '_' num2str(prey_id) '_predator']) = [predator_score_1 predator_score_2 predator_score_both];      
      toc
    end
  ...
  for prey_id_mask = 1:numel(prey_ids)
    ...
    xx{1} = table2array(bv_data(:, [prey_bv_cols]));
    xx{2} = table2array(bv_data(:, [predator_bv_cols]));
    test_1 = computeBasicVar(xx(1), net_data, 10, num_perm);
    test_2 = computeBasicVar(xx(2), net_data, 10, num_perm);
    test_both = computeBasicVar(xx, net_data, 10, num_perm);
    results.(['prey_' num2str(prey_id)]) = [test_1 test_2 test_both];
    toc
  end
  ...
  for predator_id = predator_ids
    ...
    test_2 = computeBasicVar(xx(2), net_data, 10, num_perm);
    test_both = computeBasicVar(xx, net_data, 10, num_perm);
    results.(['predator_' num2str(predator_id)]) = [test_1 test_2 test_both];
    toc
  end
  ...
  predator_both_rep_matrix = zero_mat;
  prey_both_rep_matrix = zero_mat;
  for aId = predator_ids
    for bId = prey_ids
      ...
      if isfield(results, preyField) && isfield(results, predatorField)
        ...
        prey_both_rep_matrix(aId+1, bId+1) = results.(preyField)(3);
        predator_both_rep_matrix(aId+1, bId+1) = results.(predatorField)(3);
      end
    end
  end
  ...
  subplot(2, 3, 5);
  h4 = heatmap(prey_both_rep_matrix);
  h4.Title = 'Prey Self+Partner Rep Matrix';
  h4.XLabel = 'BId';
  h4.YLabel = 'AId';
  h4.ColorbarVisible = 'on';
  h4.CellLabelFormat = '%0.2f';
  ...
  subplot(2, 3, 6);
  h4 = heatmap(predator_both_rep_matrix);
  h4.Title = 'Predator Self+Partner Rep Matrix';
  h4.XLabel = 'BId';
  h4.YLabel = 'AId';
  h4.ColorbarVisible = 'on';
  h4.CellLabelFormat = '%0.2f';
  ...
  saveas(gcf, ['interaction_heatmaps_annotated' suffix '.png']);
  ...
  for i = 1:length(prey_indices)
    ...
    if isfield(results, preyField)
      ...
      prey_both_rep(i) = results.(preyField)(3);
  ...
  for j = 1:length(predator_indices)
    ...
    if isfield(results, predatorField)
      ...
      predator_both_rep(j) = results.(predatorField)(3);
    end
  end
  ...
  figure;
  subplot(2, 1, 1);
  bar([prey_self_rep, prey_partner_rep, prey_both_rep]);
  title('Prey Self vs. Partner vs. combined Representation');
  legend('Self Rep', 'Partner Rep', 'Combined');
  set(gca, 'XTickLabel', arrayfun(@num2str, prey_indices, 'UniformOutput', false));
  xlabel('Prey Index');
  ylabel('Representation Value');

  subplot(2, 1, 2);
  bar([predator_self_rep, predator_partner_rep predator_both_rep]);
  title('Predator Self vs. Partner vs. combined Representation');
  legend('Self Rep', 'Partner Rep', 'Combined');
  ...
  result_dict.prey_both_rep_matrix = prey_both_rep_matrix;
  result_dict.predator_both_rep_matrix = predator_both_rep_matrix;
  ...
  result_dict.prey_both_rep_overall = prey_both_rep;
  result_dict.predator_both_rep_overall = predator_both_rep;
  

  save(['./results/'...
    'non_selective_partner_rep_result' suffix '.mat'], 'result_dict');
end