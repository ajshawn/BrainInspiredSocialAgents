...
def load_serial_results(results_dir):
    """
    Loads all cumulative CSV files from results_dir.
    Assumes each file is named like "<pair>_serial_results.pkl".
    Returns a combined DataFrame with a new column "pair" extracted from the filename.
    """
    csv_files = glob.glob(os.path.join(results_dir, "*_serial_results.pkl"))
    df_list = []
    for f in csv_files:
        try:
            df = pd.read_csv(f)
            pair_name = os.path.basename(f).replace("_serial_results.pkl", "")
            # df["pair"] = pair_name
...
def load_cross_results(cross_dir):
  ...
  Returns a DataFrame.
  """
  csv_files = glob.glob(os.path.join(cross_dir, "*_serial_results.pkl"))
  dfs = []
  for f in csv_files:
    try:
      df = pd.read_csv(f)
...
def load_non_cross_results():
  ...
  These files are stored in:
     /home/mikan/e/Documents/GitHub/social-agents-JAX/results/<arena_name>/pickles/serial_results_dict.pkl
  ...
  for arena_name, mix_abbr in zip(training_arena_name_list, mix_arena_abbr_list):
    file_path = os.path.join(base_path, arena_name, "pickles", "serial_results_dict.pkl")
...
def plot_combined_heatmap(combined_df, metric="pred_stuck_rate", fmt=".0%"):
  ...
  if 'mean' not in metric:
    ...
    combined_df[mean_metric] = combined_df[metric].apply(lambda x: np.nanmean(x))
  ...
  plt.tight_layout()
  # Save the figure
  plt.savefig(os.path.join('./serial_result_figures', f"combined_heatmap_{metric}.png"), dpi=300)
  plt.show()
...
if __name__ == '__main__':
  ...
  if (os.path.exists('./serial_result_figures/combined_serial_results.pkl')) and load_old:
    combined_df = pd.read_pickle('./serial_result_figures/combined_serial_results.pkl')
  else:
    cross_dir = os.path.expanduser("~/Documents/GitHub/social-agents-JAX/results/mix/analysis_results")
    cross_df = load_serial_results(cross_dir)
    non_cross_df = load_non_cross_results()
    combined_df = combine_results(cross_df, non_cross_df)

    if not os.path.exists('./serial_result_figures'):
      os.makedirs('./serial_result_figures')
    combined_df.to_pickle('./serial_result_figures/combined_serial_results.pkl', )

  # Now extract the mean of the stuck rate from stuck indicator and put the mean in the column.
  combined_df[['mean_pred_stuck_rate', 'mean_prey_stuck_rate']] = combined_df['stuck_indicator'].apply(
    lambda x: pd.Series(np.nanmean(np.hstack(x), axis=1)) if isinstance(x, list) else pd.Series([np.nan, np.nan])
  )

  # Now we retrieve the num_acorn, num_apple, num_catch from the data and put the mean in the column.
  combined_df[['mean_pred_reward', 'mean_prey_reward']] = combined_df['rewards'].apply(
    lambda x: pd.Series(np.nanmean(np.vstack(x), axis=0)) if isinstance(x, list) else pd.Series([np.nan, np.nan])
  )
  num_apple_list, num_acorn_list = [], []
  combined_df['mean_catch'] = combined_df['mean_pred_reward'] * 1000 # episode length is 1000
  for rid, row in combined_df.iterrows():
    prey_rew = np.vstack(row['rewards'])[:,1]
    apple = prey_rew == 1
    acorn = prey_rew == 6
    num_apple = np.sum(apple) / len(apple) * 1000
    num_acorn = np.sum(acorn) / len(acorn) * 1000
    num_apple_list.append(num_apple)
    num_acorn_list.append(num_acorn)
  combined_df['mean_num_apple'] = num_apple_list
  combined_df['mean_num_acorn'] = num_acorn_list

  # Now mean stamina
  combined_df[['mean_pred_stamina', 'mean_prey_stamina']] = combined_df['stamina'].apply(
    lambda x: pd.Series(np.nanmean(np.vstack(x), axis=0)) if isinstance(x, list) else pd.Series([np.nan, np.nan])
  )
  ...
  metrics = [
    'mean_pred_stuck_rate',
    'mean_prey_stuck_rate',
    'mean_pred_reward',
    'mean_prey_reward',
    'mean_num_apple',
    'mean_num_acorn',
