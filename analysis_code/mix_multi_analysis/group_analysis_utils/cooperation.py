...
def apple_frames_segmented(pos: np.ndarray, rew: np.ndarray, death: Dict[int, np.ndarray],
                           prey: Sequence[int], safe_grass_locs: set,
                           active_segments: List[Tuple[int, int]],
                           cluster_radius: int = 3, apple_min_prey: int = 2) -> List[Dict]:
    """
    Detects frames within the provided active segments where at least `apple_min_prey`
    prey are within `cluster_radius` of each other while collecting an apple,
    not on safe grass.

    Args:
        pos: Agent positions (T, A, 2).
        rew: Agent rewards (T, A).
        death: Dictionary of death labels {agent_id: np.ndarray (T,)}.
    ...
    Returns:
        ...
        meeting the criteria within any of the active segments. Each dictionary
...
def apple_periods_segmented(pos: np.ndarray, rew: np.ndarray, death: Dict[int, np.ndarray],
                             prey: Sequence[int], safe_grass_locs: set,
                             active_segments: List[Tuple[int, int]],
    ...
    """
    Detects periods of collective apple collection within the provided active
    segments.
    ...
    Args:
        ...
        participation_threshold: The minimum fraction of the event duration
                                 an agent must participate to be included.

    Returns:
        A list of dictionaries, where each dictionary represents a collective
        apple collection period within any of the active segments. Each dictionary
        has the following keys:
        - 'time_start': The start timestep of the period (aligned to episode time).
        - 'time_end': The end timestep of the period (aligned to episode time).
    ...
    """
    events = []
    for t_start_seg, t_end_seg in active_segments:
        t = t_start_seg
        while t < t_end_seg:
            ...
            duration = 1
            center_route = [center]
            agent_participation_count = {seed: 1 for seed in seeds}

            for tau in range(t + 1, t_end_seg):
...
def apple_periods_with_apple_maps_segmented(pos: np.ndarray, rew: np.ndarray,
    ...
    """
    Detects periods of collective apple collection within the provided active
    segments, considering proximity to apple tiles.
...
def analyze_collective_apple_behavior_segmentedanalyze_collective_apple_behavior_segmented(pos: np.ndarray, rew: np.ndarray,
    ...
    """
    Analyzes collective apple collection behavior within the provided active
    segments.
    ...
    Returns:
        A dictionary containing the analysis results for the active segments,
        with the following keys:
        - 'apple_frames': A list of dictionaries representing single frames
                          meeting the criteria.
        - 'apple_periods': A list of dictionaries representing collective
                           apple collection periods.
        - 'apple_periods_with_maps': A list of dictionaries representing
                                      collective apple collection periods
                                      considering apple tiles.
    """
    apple_frame_events = apple_frames_segmented(pos, rew, death, prey, safe_grass_locs,
                                                active_segments, cluster_radius)
    apple_period_events = apple_periods_segmented(pos, rew, death, prey, safe_grass_locs,
                                                  active_segments, cluster_radius,
                                                  apple_period_min_len, apple_period_gap,
                                                  participation_threshold)
    apple_period_map_events = apple_periods_with_apple_maps_segmented(
        pos, rew, death, prey, safe_grass_locs, apple_tiles, active_segments,
        cluster_radius, apple_period_min_len, apple_period_gap, participation_threshold
    )
    return {
...
if __name__ == '__main__':
    ...
    for fname in files:
        ...
        # Analyze the entire episode using the segmented function
        apple_events = analyze_collective_apple_behavior_segmented(
            pos_ep, rew_ep, death_labels_ep, prey_ids, safe_tiles, apple_tiles,
            active_segments, cluster_radius=3, apple_period_min_len=5,
            apple_period_gap=2, participation_threshold=1 / 3
        )
        # Store the results for the entire episode (as the segmented function handles segments)
    ...
    if not os.path.exists('example'):
        os.makedirs('example')
    df.to_csv('example/apple_analysis_results_example_segmented.csv', index=False)