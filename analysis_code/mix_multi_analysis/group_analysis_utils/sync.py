...
def _compute_pairwise_sync_score(
  ...
  if time_start < 0 or time_end > positions.shape[0]:
    print(f"Warning: Time window {time_start} to {time_end} is out of bounds for positions array. {positions.shape}")
    raise ValueError("Time window is out of bounds for the positions array.")

  time_end = positions.shape[0] - 1 if time_end == positions.shape[0] else time_end
  ...
  for p1_idx, p1 in enumerate(participants):
    for p2_idx in range(p1_idx + 1, len(participants)):
      # if p1_idx == 3 and p2_idx == 4:
      #   print(f"Debugging pair: {p1_idx}, {p2_idx}")
      ...
      for t in range(vel.shape[0]):
        # TODO: At this moment, we are not removing death period but still calculate a value for it.
        ...
        try:
          ...
          print(f"Error computing cosine for velocities at time {time_start + t}: {e}")
          cos_v = np.nan
      ...
      # if np.isnan(pair_scores[pair]):
      #   print(f"Warning: NaN score for pair {pair} in time window {time_start} to {time_end}.")
        # Handle NaN values if necessary, e.g., by replacing them with a specific value or logging

  # if np.isnan(list(pair_scores.values())).any():
  #   print(f"Warning: NaN values found in pair scores for time window {time_start} to {time_end}.")
    # Handle NaN values if necessary, e.g., by replacing them with a specific value or logging
  return pair_scores
