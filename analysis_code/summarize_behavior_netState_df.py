...
def process_pair(predator_id, prey_id, video_path):
  ...
  for eId in range(1, 101):
    ...
    approaching_predator = [distance_to_predator[i] < distance_to_predator[i - 1] for i in
                            range(1, len(distance_to_predator))]
    ...
    logits = np.array([info['logits'] for info in results])
    action_prob_0 = sp.special.softmax(logits[:, 0], axis=1)
    action_prob_1 = sp.special.softmax(logits[:, 1], axis=1)
    for agent_id, tmp in enumerate([action_prob_0, action_prob_1]):
      for i in range(8):
        if f'actions_prob_{agent_id}_{i}' not in info_dict:
          info_dict[f'actions_prob_{agent_id}_{i}'] = []
        info_dict[f'actions_prob_{agent_id}_{i}'].extend(tmp[:, i])
    ...
    if len(np.where(rewards[:, 1] > 1)[0]):
      print(f'Found acorn collection in {title} {eId} with {len(np.where(rewards[:, 1] > 1)[0])} acorns collected')
    t_respawn = t_catch + 21
    t_respawn = np.insert(t_respawn, 0, 0)

    # Now fill the temporal info to the info_dict
    keys = ['alive', 'on_grass', 'off_grass', 'apple', 'acorn', 'catch']
    ...
    acorn_vec = np.zeros(len(rewards))
    ...
    for ti in t_catch:
      catch_vec[ti] = 1
    for key, vec in zip(keys, [alive_vec, on_grass_vec, off_grass_vec, apple_vec, acorn_vec, catch_vec]):
...
def main():
  ...
  for video_path in video_paths:
    ...
    #     process_pair(predator_id, prey_id, video_path)
