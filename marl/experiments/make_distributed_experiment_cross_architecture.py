...
def make_distributed_experiment_cross_architecture(
  ...
  def build_learner(
    ...
    if primary_learner is None:
      ...
      dirs = checkpointing_config.directory.split(",") # first encoder, second lstm
      ...
      checkpointer.restore(ckp=ckp_map[0])
      params = unfreeze(learner._combined_states.params)
      ...
      checkpointer.restore(ckp=ckp_map[1])
      cache = unfreeze(temp_learner._combined_states.params)

      # Selectively overwrite
      for k in ['impala_network/~/lstm/linear', 'impala_network/~/policy', 'impala_network/~/value_layer']:
          if k in params:
              for k_ in params[k]:
                  params[k][k_] = cache[k][k_]

      learner._combined_states = learner._combined_states._replace(params=params)
