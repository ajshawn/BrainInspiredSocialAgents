...
def popart_impala_loss(
  ...
  def loss_fn(params: hk.Params, popart_state: rlax.PopArtState,
    ...
    # Unroll current policy over observations
    (logits, norm_values,
     hidden_features,_), _ = network.unroll_fn(params, observations,
                                             initial_state)
...
def impala_loss(
  ...
  def loss_fn(params: hk.Params, sample: types.TrainingData) -> jnp.DeviceArray:
    ...
    # Unroll current policy over observations.
    (logits, values, _,_), _ = network.unroll_fn(params, observations,
                                               initial_state)
...
def batched_popart_impala_loss_head_entropy(
    network: types.RecurrentNetworks,
    popart_update_fn,
    *,
    discount: float,
  ...
  def loss_fn(params: hk.Params, popart_state: rlax.PopArtState,
    ...
    # Unroll current policy over observations
    (logits, norm_values,
     hidden_features,attn_weights), _ = unroll_fn(params, observations, initial_state)
    #attn_weights
    # Compute importance sampling weights: current policy / behavior policy
    rhos = categorical_importance_sampling_ratios(logits[:, :-1],
                                                  behaviour_logits[:, :-1],
    ...
    # Flatten temporal dimensions if present: [T, B, ..., H, N] -> [-1, H, N]
    ...
    # Compute pairwise squared distances between heads
    pairwise_diff = (attn1 - attn2) ** 2       # [B, H, H, N]
    # Sum over feature dim (N) to get head-pair distance
    sum_dist = jnp.sum(pairwise_diff,axis = -1)  # [B, H, H]
    # Encourage head diversity: exp(-distance) is small for large distance
    head_entropy_loss = jnp.mean(jnp.exp(-sum_dist))  # scalar
    head_entropy_loss *= head_entropy_cost
...
def batched_art_impala_loss_head_entropy(
    network: types.RecurrentNetworks,
    popart_update_fn,
    *,
    discount: float,
  ...
  def loss_fn(params: hk.Params, popart_state: rlax.PopArtState,
    ...
    # Sum over feature dim (N) to get head-pair distance
    sum_dist = jnp.sum(pairwise_diff,axis = -1)  # [B, H, H]
    #jax.debug.print("sum_dist: {} {}", jnp.max(sum_dist), jnp.min(sum_dist))
    # Encourage head diversity: exp(-distance) is small for large distance
    head_entropy_loss = jnp.mean(jnp.exp(-sum_dist))  # scalar
    head_entropy_loss *= head_entropy_cost
...
def batched_art_impala_loss_head_mse(
    network: types.RecurrentNetworks,
    popart_update_fn,
    *,
    discount: float,
  ...
  def loss_fn(params: hk.Params, popart_state: rlax.PopArtState,
    ...
    # Compute sum per item map
    item_sum = jnp.sum(item_flat, axis=-1, keepdims=True)  # [B, T, item_types, 1]

    # Normalize only where sum > 0, else keep zeros
    item_flat_normalized = jnp.where(
        item_sum > 0,
        item_flat / item_sum,
        0.0
    )  # [B, T, item_types, 121]
    ...
    squared_error = (attn_probs - item_flat_normalized) ** 2  # [B, T, item_types, 121]
    squared_error = jnp.sum(squared_error, axis=-1)  # [B, T, item_types]

    # Only keep loss where item_sum > 0
    mask = (item_sum.squeeze(-1) > 0).astype(jnp.float32)  # [B, T, item_types]

    # Apply mask
    masked_mse = squared_error * mask

    # Mean over valid positions only
    mse_loss = jnp.sum(masked_mse) / jnp.maximum(jnp.sum(mask), 1.0)

    # Multiply by scaling factor
    mse_loss *= head_mse_cost  # reuse scaling factor if same weight


    # Combine weighted sum of actor & critic losses, averaged over the sequence
    critic_loss *= baseline_cost
    pi_entropy *= entropy_cost
    mean_loss = pg_loss + critic_loss + pi_entropy + mse_loss 
...
def batched_art_impala_loss_reward_predict(
    network: types.RecurrentNetworks,
    popart_update_fn,
    *,
    discount: float,
  ...
  def loss_fn(params: hk.Params, popart_state: rlax.PopArtState,
    ...
    critic_loss *= baseline_cost
    pi_entropy *= entropy_cost
    mean_loss = pg_loss + critic_loss + pi_entropy + pred_loss 
