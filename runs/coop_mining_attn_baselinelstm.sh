...
CUDA_VISIBLE_DEVICES=${GPUS} python cross_train_lstm.py  --async_distributed --available_gpus ${GPUS} --num_actors 16 \
  --algo_name PopArtIMPALA_attention --positional_embedding learnable --add_selection_vector True\
  ...
  --experiment_dir ${EXP_DIR_PREFIX} --ckp_map ${ckp_map} --freeze_encoder True \
